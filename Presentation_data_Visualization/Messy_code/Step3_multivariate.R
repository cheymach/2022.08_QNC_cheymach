...
#got VERY stuck here! Lots of trouble. Seems like PCA fully explains the variance with 1 PCA. Also can't make dendrogram work. 
...
mydata.df <- mutate(log2.cpm.filtered.norm.df,
                    NPC.AVG = (NPC + NPC_2)/2, 
                    WTC11.AVG = (WTC11 + WTC11_2)/2,
                    #now make columns comparing each of the averages above that you're interested in
                    LogFC = (NPC.AVG - WTC11.AVG)) %>% #note that this is the first time you've seen the 'pipe' operator
  mutate_if(is.numeric, round, 2)
